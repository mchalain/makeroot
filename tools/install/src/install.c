...
/* Copy files and set their permission modes and, if possible,
  ...
  -z
    use gzip compressor on the installed files
...
static void gzip PARAMS ((const char *path));
...
/* If nonzero, gzip executable files after copying them. */
static int gzip_files;
...
main (int argc, char **argv)
{
  ...
  if (dir_arg && (strip_files || gzip_files))
    error (1, 0,
...
change_attributes (const char *path)
{
  ...
  /* for cross compilation the installation is done by the user inside
   *  a virtual root directory; The owner is changed during the image
   * build.
   */
...
/* use gzip compressor from the file PATH. */

static void
gzip (const char *path)
{
  int pid, status;
  char cmd[256];
  ...
  switch (pid)
    ...
    case 0:			/* Child. */
      sprintf(cmd, "gzip");
      execlp (cmd, cmd, "-9", "-f", path, NULL);
      error (1, errno, _("cannot run gzip"));
      break;
    default:			/* Parent. */
      /* Parent process. */
...
strip (const char *path)
{
  ...
  switch (pid)
    ...
    case 0:			/* Child. */
      ...
      if (cross_compile)
        sprintf(cmd, "%sstrip", cross_compile);
      else
        sprintf(cmd, "strip");
...
\n\
  ...
  -z                  use gzip compressor on the installing file\n\
\n\
"));
      printf (_("\
