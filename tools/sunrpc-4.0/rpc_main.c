...
//MARPOSS
char function_main[16] = "main";
const char function_svcmain[] = "svc_main";
...
open_input(const char *infile, const char *define)
{
	...
	find_cpp();
	sprintf(szTemp, "%s %s %s %s > %s\n", CPP, CPPFLAGS, define, infile, szTempFile);
	//printf(szTemp);

	system(szTemp);
	fin = fopen(szTempFile, "r");
	if (fin == NULL) {
		perror("fopen\n");
		crash();
	}
  ...
  for (i = 1; i < argc; i++)
      ...
      else
	{
	  for (j = 1; argv[i][j] != 0; j++)
		...
		case 'z':
		  strcpy(function_main, function_svcmain);
		  break;
...
usage (void)
{
  fprintf (stderr, "MARPOSS rpcgen for Windows Version 1.0.0. 2008-2008 - from sunrpc glibc 2.7\n\n");
...
options_usage (void)
{
  ...
  f_print (stderr, "-z\t\tgenerate svc_main instead main in xxx_svc.c\n");	//MARPOSS
